...
## 6. 附：凭据文件 —— key 和 IV 是同一个东西

### 6.1 结论是抄的，但验证过

`~/.qoder/.auth/user` 的格式（上游公开实现里就是这么写的）：

```python
key = machine_id[:16]        # 36 字符 UUID 的前 16 字节 —— 恰好是一个 16 字节 ASCII 串
AES-128-CBC(key, iv=key)     # IV 复用同一个值
base64(PKCS7(明文))
```

**这个 IV 不是"解"出来的**：它来自已有的开源实现，照抄即通。协议那部分是这次逆向的，
凭据解密这部分不是 —— 这点得说清楚，别把别人的成果算在自己头上。

但照抄不等于对，所以做了一个判别力很强的验证：把解出来的明文按同一方案**重新加密**，
看能不能逐字节还原磁盘上的文件。
...
plain = unpad(dec.update(blob) + dec.finalize())
data = json.loads(plain)                      # 能出合法 JSON

# 再加密回去，与原文件比对
reblob = Cipher(algorithms.AES(key), modes.CBC(key)).encryptor().update(pad(plain)) + ...
assert reblob == blob                          # ← intl / CN 两边都是 True
```

**逐字节相同**这一步很关键：如果 IV 是随机的、或密文头部拼了随机数、或另外挂了 MAC，
重加密**必然**不相等。所以这条结论现在是实测钉死的，而不是"上游说它是"。
（顺带：intl 与 CN 两个安装的 `machine_id` 是同一个值，只有里面的 JSON 不同。）

### 6.2 没有现成代码时，怎么自己搞出 key / IV

固定套路，而且**不需要破 AES**：

1. **看块对齐**：base64 解出来是 16 的倍数 → AES 分组（CBC 或 ECB）。
2. **确定性检测**：同一账号在同一台机器上重新登录两次，比较密文。前缀相同 → 没有随机 IV
   （ECB 或固定 IV）。这一步先把"随机 IV"排除掉，候选空间立刻塌下来。
3. **候选穷举**：目录里那个 36 字符 UUID，前 16 字节恰好是一个 16 字节 ASCII 串 ——
   全目录唯一像密钥的东西。AES 本身不可能被猜破，**但密钥要是就写在密文旁边，那就不用破**。
   判据是两条同时成立：PKCS7 去填充通过 **且** 解出来是合法 JSON（误判概率可忽略）。
4. **进程内挂钩子（最通用）**：直接看客户端调 `crypto.createDecipheriv` 时传进去的 key/iv。
   Node 客户端钩 `require('crypto').createDecipheriv`（`NODE_OPTIONS=--require`），
   Bun 编译的单文件二进制用 `BUN_OPTIONS=--preload`。

第 4 条跟第 4 章抓 `X-Model-Key` 是**同一招**：不要猜参数，去看它自己怎么调的。

### 6.3 顺带看一眼这个设计（以及它真正的危害）

- **这不叫加密，叫本地混淆。** 密钥是客户端自己编的，从头到尾没跟服务端交换过 —— 密文和密钥
  躺在同一个目录里，只隔一个文件名。它防的不是攻击者，是**顺手 `cat` 一下**。
- **"机器绑定"两头都是虚构的**：一头密钥是自己编的，另一头服务端**不校验**机器指纹
  （发随机 `machine_id` / `machinetoken` 照样受理、照样出话，见第 4 章抓到的头）。
- **真正会咬人的是安全错觉**：用户看到"加密过"，就会把 `.auth` 目录同步到网盘、塞进备份、
  在机器之间拷贝 —— 登录态跟着一起走，而用户以为有保护。**它不提供保护，却让人以为有保护**，
  这比明说"文件里就是明文"更危险。
- 公平地说，他们为什么敢这么写：公开客户端**不存在**"只有客户端知道、服务端不知道"的秘密
  （和硬编码 `SECRET` 是同一个物理限制）。承认没有本地秘密、做点混淆，这个判断本身没错；
  错在**没把话说清楚** —— 文档里写"以本机机器标识为密钥加密存储"，读起来像有保护，
  实际含义是"自己锁自己"。
- 正确做法：密钥留在服务端（或系统密钥链，客户端只拿不可导出的引用），本地落地用
  **随机 IV + KDF**，再加完整性校验 —— 至少"拷贝目录"不会直接等于"拷贝一个人的登录"。
